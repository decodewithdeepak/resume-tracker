...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    if (!authed && req.method === 'POST') {
        // Parse body manually (Next.js does not parse body in getServerSideProps)
    ...
    if (authed) {
        await dbConnect();
        logs = await VisitLog.find({}).sort({ timestamp: -1 }).lean();
    }
...
export default function Visits({ authed, logs }: VisitsProps) {
    ...
    const byBrowser: Record<string, number> = logs.reduce((acc: Record<string, number>, l: any) => {
